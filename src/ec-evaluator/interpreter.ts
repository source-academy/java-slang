...
export const cmdEvaluators: { [type: string]: CmdEvaluator } = {
  ...
  NormalClassDeclaration: (
    ...
    const superclass: Class | undefined = command.sclass
      ? environment.getClass(command.sclass)
      : undefined
  ...
  Assignment: (command: Assignment, _environment: Environment, control: Control, _stash: Stash) => {
    control.push(instr.assmtInstr(command))
  ...
  [InstrType.ASSIGNMENT]: (
    ...
    variable.value.kind === StructType.VARIABLE
  ...
  [InstrType.RES]: (
    ...
    if (varOrClass.kind === StructType.VARIABLE) {
      ...
      let staticField = classOfField.staticFields.find(
        f => f.variableDeclaratorList[0].variableDeclaratorId === command.name
      )
      let instanceField = classOfField.instanceFields.find(
        f => f.variableDeclaratorList[0].variableDeclaratorId === command.name
      )
      while (!staticField && !instanceField && classOfField.superclass) {
        classOfField = classOfField.superclass
        staticField = classOfField.staticFields.find(
          f => f.variableDeclaratorList[0].variableDeclaratorId === command.name
        )
        instanceField = classOfField.instanceFields.find(
          f => f.variableDeclaratorList[0].variableDeclaratorId === command.name
        )
      }
  ...
  [InstrType.DEREF]: (
    ...
    const varOrClass = stash.pop()! as Variable | Class
    const value =
      varOrClass.kind === StructType.CLASS
        ? varOrClass
        : // Variable can store variable now.
  ...
  [InstrType.RES_TYPE]: (
    ...
    if (value.kind === 'Literal') {
      ...
      if (isQualified(value.name)) {
        ...
        control.push(
          instr.resTypeInstr(node.exprNameNode(nameParts[0], command.srcNode), command.srcNode)
        )
        return
      }
  ...
  [InstrType.RES_OVERLOAD]: (
    ...
    if (isInstance(closure.decl as MethodDeclaration)) {
      ...
      while (
        control.peekN(n) &&
        (isNode(control.peekN(n)!) ||
          (control.peekN(n)! as Instr).instrType !== InstrType.INVOCATION)
      ) {
        n++
      }
