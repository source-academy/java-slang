  ...
  createClass(options: {
    ...
    let constantPool: ConstantInfo[] = [
      ...
      { tag: 7, nameIndex: 2 }, // superclass
      {
        tag: 1,
        length: 16,
        value: options.superClass ? options.superClass?.getName() : 'java/lang/Object'
      }, // superclass name
      { tag: 7, nameIndex: 4 },
      {
        tag: 1,
    ...
    const methods = options.methods
      ...
      : []
    const loader = options.loader ?? new TestClassLoader(new TestSystem(), 'test/', null)
    ...
    const interfaces = options.interfaces
      ? options.interfaces.map((interfaceCls: ReferenceClassData, index: number) => {
