...
export default abstract class AbstractClassLoader {
   ...
   * @returns
   * */
  protected linkClass(cls: ClassFile, protectionDomain?: JvmObject): ImmediateResult<ClassData> {
  ...
  protected load(className: string): ImmediateResult<ClassData> {
    const path = (this.classPath ? this.classPath + '/' + className : className) + '.class'
...
export class ApplicationClassLoader extends AbstractClassLoader {
  ...
  constructor(nativeSystem: AbstractSystem, classPath: string, parentLoader: AbstractClassLoader) {
