...
export const info2Attribute = (info: AttributeInfo, constantPool: ConstantPool): IAttribute => {
  const name = (constantPool.get(info.attributeNameIndex) as ConstantUtf8).get()

  switch (name) {
    case 'ConstantValue':
      return {
        name,
        constantvalue: constantPool.get((info as ConstantValueAttribute).constantvalueIndex)
      } as ConstantValue
    case 'Code':
    ...
    case 'InnerClasses':
      ...
      ;(info as InnerClassesAttribute).classes.forEach(element => {
        innerclasses.push({
          innerClass: constantPool.get(element.innerClassInfoIndex) as ConstantClass,
          outerClass:
            element.outerClassInfoIndex === 0
              ? null
              : (constantPool.get(element.outerClassInfoIndex) as ConstantClass),
          innerName:
            element.innerNameIndex === 0
              ? null
              : (constantPool.get(element.innerNameIndex) as ConstantUtf8).get(),
    ...
    case 'EnclosingMethod':
      const cls = constantPool.get((info as EnclosingMethodAttribute).classIndex) as ConstantClass
    ...
    case 'Signature':
      const signature = (
        constantPool.get((info as SignatureAttribute).signatureIndex) as ConstantUtf8
      ).get()
      return {
        name,
        signature
    ...
    case 'LocalVariableTable':
      ...
      ;(info as LocalVariableTableAttribute).localVariableTable.forEach(element => {
        localVarTable.push({
          startPc: element.startPc,
          length: element.length,
          name: (constantPool.get(element.nameIndex) as ConstantUtf8).get(),
          descriptor: (constantPool.get(element.descriptorIndex) as ConstantUtf8).get(),
          index: element.index
        })
      })
      return {
    ...
    case 'LocalVariableTypeTable':
      ...
      ;(info as LocalVariableTypeTableAttribute).localVariableTypeTable.forEach(element => {
        localVarTypeTable.push({
          startPc: element.startPc,
          length: element.length,
          name: (constantPool.get(element.nameIndex) as ConstantUtf8).get(),
          signature: (constantPool.get(element.signatureIndex) as ConstantUtf8).get(),
          index: element.index
        })
      })
    ...
    case 'BootstrapMethods':
      ...
      ;(info as BootstrapMethodsAttribute).bootstrapMethods.forEach(element => {
        ...
        bootstrapMethods.push({
          bootstrapMethodRef: constantPool.get(element.bootstrapMethodRef) as ConstantMethodHandle,
          bootstrapArguments
        })
      })
      return {
    ...
    case 'SourceFile':
      return {
        name,
        sourceFile: constantPool.get((info as SourceFileAttribute).sourcefileIndex) as ConstantUtf8
      } as SourceFile
    case 'Synthetic':
      return {
        name
