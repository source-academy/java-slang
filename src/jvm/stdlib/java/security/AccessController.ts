...
const doPrivileged = (thread: Thread, locals: any[]) => {
  ...
  const methodRes = actionCls.resolveMethod('run()', 'Ljava/lang/Object;', thread.getClass())
  ...
  thread.invokeStackFrame(
    new InternalStackFrame(methodRef.getClass(), methodRef, 0, [action], (ret: JvmObject) => {
...
const functions = {
  ...
  'doPrivileged(Ljava/security/PrivilegedExceptionAction;)Ljava/lang/Object;': doPrivileged,
  ...
  'doPrivileged(Ljava/security/PrivilegedAction;)Ljava/lang/Object;': (
    ...
    if (!method) {
      thread.throwNewException('java/lang/NoSuchMethodException', 'run()Ljava/lang/Object;')
      return
    }
   ...
   * @param locals
   */
  'getStackAccessControlContext()Ljava/security/AccessControlContext;': (thread: Thread) => {
    logger.warn(
