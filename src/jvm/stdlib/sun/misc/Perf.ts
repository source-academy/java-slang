...
const functions = {
  'createLong(Ljava/lang/String;IIJ)Ljava/nio/ByteBuffer;': (thread: Thread, locals: any[]) => {
    const value = locals[4] as bigint

    const bbRes = thread.getMethod().getClass().getLoader().getClass('java/nio/DirectByteBuffer')
