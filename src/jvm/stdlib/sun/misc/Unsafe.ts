...
import { ReferenceClassData, ArrayClassData, ClassData } from '../../../types/class/ClassData'
...
function getFieldInfo(thread: Thread, unsafe: JvmObject, obj: JvmObject, offset: bigint): any {
  // obj is a class obj
...
const functions = {
  ...
  'arrayBaseOffset(Ljava/lang/Class;)I': (thread: Thread) => {
   ...
   * @param locals
   */
  'objectFieldOffset(Ljava/lang/reflect/Field;)J': (thread: Thread, locals: any[]) => {
  ...
  'arrayIndexScale(Ljava/lang/Class;)I': (thread: Thread, locals: any[]) => {
    ...
    const scale = typeIndexScale((clsRef as ArrayClassData).getComponentClass())
    thread.returnStackFrame(scale)
  },

  'addressSize()I': (thread: Thread) => {
  ...
  'compareAndSwapObject(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Z': (
    ...
    thread.returnStackFrame(unsafeCompareAndSwap(thread, unsafe, obj1, offset, expected, newValue))
  },

  'compareAndSwapInt(Ljava/lang/Object;JII)Z': (thread: Thread, locals: any[]) => {
    const unsafe = locals[0] as JvmObject
    const obj1 = locals[1] as JvmObject
    const offset = locals[2] as bigint
  ...
  'compareAndSwapLong(Ljava/lang/Object;JJJ)Z': (thread: Thread, locals: any[]) => {
    const unsafe = locals[0] as JvmObject
    const obj1 = locals[1] as JvmObject
    const offset = locals[2] as bigint
  ...
  'defineAnonymousClass(Ljava/lang/Class;[B[Ljava/lang/Object;)Ljava/lang/Class;': (
    ...
    const bytecode = new DataView(new Uint8Array(byteArray.getJsArray()).buffer)
    const classfile = parseBin(bytecode)

    // resolve classname
    const clsInfo = classfile.constantPool[classfile.thisClass] as ConstantClassInfo
    const clsName = classfile.constantPool[clsInfo.nameIndex] as ConstantUtf8Info
  ...
  'defineClass(Ljava/lang/String;[BIILjava/lang/ClassLoader;Ljava/security/ProtectionDomain;)Ljava/lang/Class;':
    (thread: Thread, locals: any[]) => {
      ...
      if (loadResult.status === ResultType.ERROR) {
        thread.throwNewException(loadResult.exceptionCls, loadResult.msg)
        return
      }
      thread.returnStackFrame(loadResult.result.getJavaObject())
    },

  'ensureClassInitialized(Ljava/lang/Class;)V': (thread: Thread, locals: any[]) => {
    const clsObj = locals[1] as JvmObject
  ...
  'shouldBeInitialized(Ljava/lang/Class;)Z': (thread: Thread, locals: any[]) => {
    const clsObj = locals[1] as JvmObject
    const cls = clsObj.getNativeField('classRef') as ClassData
    thread.returnStackFrame(cls.isInitialized() ? 1 : 0)
  },

  'staticFieldOffset(Ljava/lang/reflect/Field;)J': (thread: Thread, locals: any[]) => {
    const field = locals[1] as JvmObject
    const slot = field._getField('slot', 'I', 'java/lang/reflect/Field') as number
    ...
    thread.returnStackFrame64(BigInt(jsField ? clsRef.getFieldVmIndex(jsField) : -1))
  },
  ...
  'staticFieldBase(Ljava/lang/reflect/Field;)Ljava/lang/Object;': (
    ...
    const objRes = cls.getLoader().getClass('java/lang/Object') as SuccessResult<ClassData>
