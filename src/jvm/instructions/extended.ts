...
export function runWide(thread: Thread): void {
  ...
  switch (opcode) {
    ...
    case OPCODE.LLOAD:
    case OPCODE.DLOAD:
      thread.pushStack64(thread.loadLocal(indexbyte) as bigint | number) && thread.offsetPc(4)
      return
    case OPCODE.ISTORE:
    case OPCODE.ASTORE:
    ...
    case OPCODE.IINC:
      ...
      thread.storeLocal(indexbyte, ((thread.loadLocal(indexbyte) as number) + constbyte) | 0)
      thread.offsetPc(6)
      return
  }
...
export function runMultianewarray(thread: Thread): void {
  ...
  const arrayClsConstant = thread.getClass().getConstant(indexbyte) as ConstantClass
  ...
  for (let i = 0; i < dimensions; i++) {
    ...
    if (dim < 0) {
      thread.throwNewException('java/lang/NegativeArraySizeException', 'Negative array size')
      return
    }
  ...
  const res = arrayCls.instantiate()
  ...
  for (let i = 1; i < dimensions; i++) {
    ...
    const classResolutionResult = thread.getClass().getLoader().getClass(currentType)
    if (classResolutionResult.status === ResultType.ERROR) {
      thread.throwNewException(classResolutionResult.exceptionCls, classResolutionResult.msg)
      return
    }
